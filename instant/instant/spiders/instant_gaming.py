...
class InstantGamingSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        item = InstantItem()
        item["Title"] = response.xpath('//div[@class = "infos mainshadow"]/h1/text()').extract()
        item["Plateform"] = response.xpath('//div[@class = "subinfos"]/a/text()').extract()[1]
        item["Release"] = response.xpath('//div[@class =  "release"]/span/text()').extract()
        item["Tags"] = response.xpath('//div[@class = "tags"]/a/text()').extract()
        item["Flat_price"] = response.xpath('//div[@class = "retail"]/span/text()').extract()
        item["Discount"] = response.xpath('//div[@class = "discount"]/text()').extract()
        ...
        item["Publisher"] = response.xpath('//div[@class =  "publisher"]/a/text()').extract()
        return item
